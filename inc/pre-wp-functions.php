 ...
 * @return string
 */
function atozsites_file_cache( $buffer, $flags ) {
	global $post;

	$cache_dir = atozsites_get_cache_dir();
	...
	if ( ! file_exists( $cache_dir ) ) {
		...
		$buffer = apply_filters( 'atozsites_pre_cache_buffer', $buffer );
	}

	$url_path = atozsites_get_url_path();
	...
	if ( ! empty( $GLOBALS['atozsites_config']['enable_gzip_compression'] ) && function_exists( 'gzencode' ) ) {
	...
	if ( function_exists( 'ob_gzhandler' ) && ! empty( $GLOBALS['atozsites_config']['enable_gzip_compression'] ) ) {
 ...
 * @return string
 */
function atozsites_get_url_path() {
 ...
 * @return string
 */
function atozsites_get_cache_path() {
	return rtrim( WP_CONTENT_DIR, '/' ) . '/cache/atozsites-simple-cache';
}
 ...
 * @since 1.0
 */
function atozsites_serve_file_cache() {
	$cache_dir = ( defined( 'atozsites_CACHE_DIR' ) ) ? rtrim( atozsites_CACHE_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/cache/atozsites-simple-cache';

	$file_name = 'index.html';

	if ( function_exists( 'gzencode' ) && ! empty( $GLOBALS['atozsites_config']['enable_gzip_compression'] ) ) {
		$file_name = 'index.gzip.html';
	}

	$path = $cache_dir . '/' . rtrim( atozsites_get_url_path(), '/' ) . '/' . $file_name;
 ...
 * @return string
 */
function atozsites_get_cache_dir() {
	return ( defined( 'atozsites_CACHE_DIR' ) ) ? rtrim( atozsites_CACHE_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/cache/atozsites-simple-cache';
}
 ...
 * @return string
 */
function atozsites_get_config_dir() {
	return ( defined( 'atozsites_CONFIG_DIR' ) ) ? rtrim( atozsites_CONFIG_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/sc-config';
}
 ...
 * @return string
 */
function atozsites_get_config_file_name() {
 ...
 * @return bool|array
 */
function atozsites_load_config() {
	if ( @file_exists( atozsites_get_config_dir() . '/config-network.php' ) ) {
		return include atozsites_get_config_dir() . '/config-network.php';
	} elseif ( @file_exists( atozsites_get_config_dir() . '/' . atozsites_get_config_file_name() ) ) {
		return include atozsites_get_config_dir() . '/' . atozsites_get_config_file_name();
	}
 ...
 * @return boolean
 */
function atozsites_url_exception_match( $exception, $regex = false ) {
